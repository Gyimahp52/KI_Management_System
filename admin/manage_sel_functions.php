...
function startNewTerm($academic_year_id, $term_number, $start_date, $end_date) {
    ...
    try {
        ...
        // Move theme assignments to historical records (including the order)
        $stmt = $pdo->prepare("
            INSERT INTO historical_school_themes (school_id, theme_id, term_id, academic_year_id, `order`)
            SELECT st.school_id, st.theme_id, st.term_id, ?, st.`order`

...
$stmt = $pdo->prepare(" SELECT s.*, c.class_name, sc.school_name FROM students s JOIN classes c ON s.class_id = c.class_id
...
function generateKEQBarChart($studentData, $studentName) {
  ...
  // Title
  $svg .= "<text x=\"" . ($width / 2) . "\" y=\"35\" text-anchor=\"middle\" font-weight=\"bold\" font-size=\"22\" fill=\"#ffffff\">Kinesthetic Emotional Intelligence Quotient (KEQ) for $studentName</text>";
  ...
  for ($i = 0; $i < count($labels); $i++) {
      ...
      $svg .= "<text x=\"" . ($x + ($xScale * 0.4)) . "\" y=\"" . ($height - $margin['bottom'] - $baseHeight / 2) . "\" text-anchor=\"middle\" font-size=\"14\" fill=\"#000000\">" . $baseData[$i] . "</text>";
      ...
      $svg .= createSVGElement('rect', [
          ...
          'width' => $xScale * 0.6,
          'height' => $improvementHeight,
          'fill' => '#2196F3'
      ]);

      // Improvement Value label (aligned to bottom)
      $svg .= "<text x=\"" . ($x + ($xScale * 0.4)) . "\" y=\"" . ($height - $margin['bottom'] - $baseHeight - 2.7) . "\" text-anchor=\"middle\" font-size=\"14\" fill=\"#ffffff\">" . $improvementData[$i] . "</text>";
      ...
      $svg .= "<text x=\"" . ($x + ($xScale * 0.4)) . "\" y=\"" . ($height - $margin['bottom'] - $baseHeight - $improvementHeight - $targetHeight / 2) . "\" text-anchor=\"middle\" font-size=\"14\" fill=\"#000000\">" . (50 - $improvementData[$i]) . "</text>";
  ...
  for ($i = 0; $i <= 100; $i += 10) {
      ...
      $svg .= "<text x=\"" . ($margin['left'] - 10) . "\" y=\"$y\" text-anchor=\"end\" font-size=\"12\" fill=\"#ffffff\">" . $i . "%</text>";
  }
  ...
  foreach ($legendItems as $index => $item) {
      ...
      $svg .= "<text x=\"" . ($legendX + 20 + ($index * $legendSpacing)) . "\" y=\"" . ($legendY + 12) . "\" font-size=\"14\" fill=\"#ffffff\">" . $item['label'] . "</text>";
  }
...
function generateSELPieChart($studentData, $studentName) {
  ...
  function competencyMap($abbreviation) {
      $map = [
  ...
  // Title
  $svg .= "<text x=\"$centerX\" y=\"40\" text-anchor=\"middle\" font-weight=\"bold\" font-size=\"22\" fill=\"white\">Social Emotional Learning Competencies for $studentName</text>";
  ...
  foreach ($selLabels as $index => $label) {
      ...
      $svg .= "<text x=\"$labelX\" y=\"$labelY\" text-anchor=\"middle\" font-size=\"14\" fill=\"black\">" . $value . " (" . number_format($percentage, 0) . "%)</text>";
  ...
  foreach ($selLabels as $index => $label) {
      ...
      $svg .= "<text x=\"" . ($legendX + 20) . "\" y=\"" . ($legendY + 12 + $legendYOffset) . "\" font-size=\"14\" fill=\"white\">" . $fullLabel . " (" . $label . ")</text>";
  }
...
function generateCharacterStrengthsBarChart($studentData, $studentName) {
  ...
  // Title
  $svg .= "<text x=\"" . ($baseWidth / 2) . "\" y=\"30\" text-anchor=\"middle\" font-weight=\"bold\" font-size=\"20\" fill=\"white\">Character Strengths by Month for $studentName</text>";
  ...
  // Centered Legend
  $legendWidth = count($csLabels) * 150; // Assuming each legend item is about 150px wide
  ...
  foreach ($csLabels as $index => $label) {
      ...
      $svg .= "<text x=\"" . ($legendX + $index * 150 + 20) . "\" y=\"" . ($legendY + 12) . "\" font-size=\"12\" fill=\"white\">" . $label . "</text>";
  }
...
#keqBarChart, #selPieChart, #csBarChart {
        width: 100%;
        height: 400px;
    }
...
@media print {
    ...
    body {
      color: black; /* Ensure text is black (color can be managed in print settings) */
    }
